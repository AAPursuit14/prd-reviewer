...
function buildCSV(reviews) {
  const headers = ["Product", "Builder/Team", "Problem statement", "Scope and feasibility", "Technical clarity", "User and feature alignment", "Verdict", "What works", "What needs work", "Next steps"];
...
export default function App() {
  ...
  const reviewAll = useCallback(async () => {
    ...
    for (let i = 0; i < queue.length; i++) {
      ...
      const item = queue[i];
      try {
        const result = await reviewPRD(item.text);
        newReviews.push({ id: item.id, label: item.label, result, error: null });
      } catch (e) {
        newReviews.push({ id: item.id, label: item.label, result: null, error: e.message });
      }
      setReviews((prev) => [...prev, newReviews[newReviews.length - 1]]);
    }
    setQueue([]);
    setProcessing(false);
  }, [queue]);
        ...
        <div>
          <div style={{ background: "#fff", border: "1.5px solid #e5e7eb", borderRadius: 12, padding: "1.25rem", marginBottom: "1.5rem" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
              ...
              <span style={{ fontSize: 15, fontWeight: 500 }}>Add a PRD to the review queue</span>
            </div>
          ...
          {queue.length > 0 && (
            <div style={{ background: "#fff", border: "1.5px solid #e5e7eb", borderRadius: 12, padding: "1.25rem" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                ...
                <button onClick={reviewAll} disabled={processing} style={processing ? BTN.disabled : BTN.primary}>
